# SOLUTION SUGGESTION: Text-to-SQL Agent Platform (BIRD Benchmark)

### Đề xuất Giải pháp Kỹ thuật | v1.1 — Updated for BIRD multi-database
...
Xây dựng một **AI Agent** cho phép người dùng (không biết SQL) đặt câu hỏi bằng ngôn ngữ tự nhiên và nhận câu trả lời chính xác từ database. Đánh giá trên **BIRD-SQL benchmark** với 70+ databases đa dạng domain.

**Quy mô dữ liệu (BIRD benchmark):**

| Đặc điểm | Giá trị |
|-----------|---------|
| Số databases | 70+ (diverse domains) |
| Số bảng (mỗi DB) | 2-20+ |
| Tổng examples | 9,430+ (question + SQL pairs) |
| Database engine | SQLite |
| Độ phức tạp SQL | Simple SELECT đến multi-table JOINs, subqueries, CTEs |
| Ngôn ngữ input | Tiếng Anh (BIRD native) + Tiếng Việt (extended) |
...
|----------|--------|----------|-----------|-------|
| **① Fine-tuned LLM** | Train LLM riêng trên schema/SQL pairs | 60-70% (BIRD) | Không | Cần dataset lớn, tốn GPU, khó maintain khi schema thay đổi, 70+ schemas khác nhau |
| **② Direct Prompting** | Gửi toàn bộ schema + question vào LLM | 50-65% (BIRD) | Không | Multi-DB schemas quá đa dạng, prompt quá dài, dễ hallucinate |
| **③ RAG + Single LLM** | Retrieve schema liên quan → LLM sinh SQL | 60-80% (BIRD) | **Phase 1** | Đơn giản, nhanh benchmark. Dùng BIRD evaluation để đo baseline accuracy |
| **④ RAG + Validation Pipeline** | Retrieve → LLM sinh SQL → code validate → execute | **70-85%** (BIRD) | **Phase 2** | Accuracy cao nhất, validation layer bắt lỗi, modular |

**3 lý do chính chọn LLM-in-the-middle Pipeline:**

**① Accuracy là ưu tiên số 1 — đánh giá bằng BIRD benchmark**
- BIRD benchmark là tiêu chuẩn công nghiệp cho Text-to-SQL evaluation.
- 70+ databases đa dạng domain → test khả năng generalization.
- Execution accuracy (EX) đo chính xác: generated SQL trả kết quả đúng hay không.
- Phase 1 dùng RAG + Single Agent để benchmark baseline. Phase 2 thêm validation pipeline.

**② Multi-database cần RAG — mỗi DB có schema riêng**
- 70+ databases với schemas khác nhau → không thể nhồi tất cả vào prompt.
- RAG retrieve schema chunks per db_id → context gọn, relevant.
- BIRD evidence field cung cấp domain hints thay cho semantic layer cố định.
...
╠══════════════════════════════════════════════════════════════════╣
║  LAYER 5: DATA                                                   ║
║  SQLite (BIRD eval) / PostgreSQL 18 (production)                                       ║
╚══════════════════════════════════════════════════════════════════╝
```
...
| **Audit Logger** | Log: who asked, what SQL, when, what result | **Bắt buộc** (compliance + evaluation tracking) |
...
| Component | Vai trò | Bắt buộc? |
|-----------|---------|-----------|
| **SQLite / PostgreSQL** | Phase 1: SQLite (BIRD eval). Phase 2+: PostgreSQL (production) | **Bắt buộc** |
...
| F2 | Schema linking | Tìm đúng bảng/cột per database (BIRD: 70+ DBs, diverse schemas) | **P0** |
...
| **Database** | Eval / Prod | **SQLite (BIRD eval)** → **PostgreSQL 18 (production)** | Phase 1: SQLite match BIRD ground truth. Phase 2+: PostgreSQL + pgvector |
...
│                                                    Grafana        │
│                                                                   │
│  SQLite (BIRD eval) / PostgreSQL 18 (production)  (xuyên suốt cả 3 phases)              │
...
Trọng số dựa trên yêu cầu Text-to-SQL multi-database (BIRD benchmark):
...
**Kết luận:** Pattern 1 (LLM-in-the-middle Pipeline) đạt điểm cao nhất nhờ accuracy và safety. Tuy nhiên, lộ trình **Pattern 2 → 1 → 3** là con đường thực tế nhất: Phase 1 benchmark trên BIRD, Phase 2 migrate sang production database, Phase 3 scale.
...
**Kết luận:** Với BIRD benchmark (70+ databases), kiến trúc modular cho phép scale per-database. Phase 1 dùng SQLite + BIRD evaluation. Phase 2+ migrate sang PostgreSQL production. Ưu tiên scale **retrieval layer** trước vì đó là bottleneck khi số databases và schema diversity tăng.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  Bước 1: Parse schema.json
    → Extract schema per database (BIRD: DDL from sqlite_master or schema field)
...
|---------|----------------------------|-------------------------------|
| **Prompt size** | Dài — toàn bộ schema + rules + examples | Ngắn — chỉ 2-5 bảng liên quan (Linker đã filter per db_id) |
