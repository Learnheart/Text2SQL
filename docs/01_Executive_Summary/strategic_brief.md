# STRATEGIC BRIEF: Text-to-SQL Agent Platform
...
Xây dựng **Text-to-SQL Agent Platform** theo kiến trúc RAG (Retrieval-Augmented Generation):

```
Người dùng → Câu hỏi tiếng Việt/Anh
       ↓
[Schema Retrieval] → Lấy metadata bảng/cột liên quan từ Vector DB
       ↓
[LLM Agent] → Sinh SQL query chính xác + giải thích
       ↓
[SQL Execution] → Thực thi trên Production DB (read-only)
       ↓
[Insight Generation] → Trả kết quả + phân tích insight
```

**Lợi thế cạnh tranh so với mua giải pháp thương mại:**
- **Kiểm soát dữ liệu**: Dữ liệu không rời khỏi hạ tầng nội bộ.
- **Tùy biến domain**: Tối ưu cho schema và nghiệp vụ riêng (banking/POS).
- **Chi phí dài hạn**: Tránh vendor lock-in; chi phí vận hành thấp hơn SaaS 40-60% sau năm thứ 2.
- **Compliance**: Tuân thủ Luật AI Việt Nam và các quy định bảo mật dữ liệu ngành tài chính.
...
|-----------|----------|----------|
| **Kỹ thuật** | Khả thi cao | SOTA accuracy trên benchmark chuẩn (Spider): ~91%. Kiến trúc RAG + Agent đã được validate bởi Snowflake, Databricks |
...
| 3 | Hoàn thành POC trên banking dataset hiện có | Tuần 5-16 | Tech Lead |
| 4 | Demo & đánh giá Go/No-Go cho Phase 2 | Tuần 17-18 | Steering Committee |

**Decision gate:** Sau Phase 1 POC, đánh giá accuracy ≥ 85% trên top 50 business queries trước khi commit Phase 2.
